...
Für den vorliegenden Text liegt der Fokus jedoch auf den drei obengenannten Faktoren.
Einerseits aufgrund ihrer dichteren Forschungslage, andererseits um Kreuzbeziehungen (zum Beispiel: Wie wirkt Unternehmensgovernance auf den Arbeitsort und die IP-Adresse bei Home-Office-Regelungen?) zu reduzieren, die hier nicht in der gebührenden Komplexität dargestellt werden könnten.
Zur Absicherung digitaler Identitäten wird eine Kombination mehrerer etablierter Protokolle und Technologien verwendet, um sowohl die Identitätsprüfung als auch die Zugriffssteuerung zu regeln (Rose et al., 2020, S. 6).
Auf diese wird in Folge genauer eingegangen.
...
Die Authentifizierung ist der Prozess, bei dem ein Anspruchsteller/ eine Anspruchstellerin (Claimant) den Besitz und die Kontrolle über einen oder mehrere Authentifikatoren nachweist, um seine Identität gegenüber einem Bestätiger (Verifier) zu bestätigen (Temoshok et al., 2025, S. 4--5). Authentifizierungsmethoden basieren traditionell, wie einleitend erwähnt, auf drei Faktoren (NIST SP 800-63 Digital Identity Guidelines, o.\,J.).
...
\subsection{Usability und Mensch-Maschine-Interaktion} (Sasse et al., 2001, S. 1-2)
...
Denn die Akzeptanz und korrekte Nutzung von MFA hängt maßgeblich davon ab, wie intuitiv und benutzerfreundlich die Systeme gestaltet sind (Cranor \& Garfinkel, 2005, S. 1).
Das führt zum Begriff der Usable Security, einer Sub-disziplin der HCI für den IT-Sicherheitskontext. ( Cranor \& Garfinkel, 2005, S. 1) % „Usable Security is the design and implementation of security mechanisms that are not only effective but also usable by their intended users.“
...
Im MFA-Kontext tritt hier die Frage nach der Aufgabe der Technologie auf (Primäraufgabe vs. Sekundäraufgabe) %\bibitem{marky2022}
(Marky et al., 2022, S. 15): MFA ist nie das eigentliche Ziel der User:innen, sondern eine Sekundäraufgabe
Es muss ein Kompromiss zwischen Sicherheit und Bequemlichkeit getroffen werden. 
% wie bereits beschrieben, aber wo? 
Mögliche Folgen eines schlecht ausverhandelten MFA-System können „Security Fatigue“ (Sicherheitsermüdung), negative Reaktionen und die Verwendung von Workarounds sein (Fagan \& Khan, 2016, S. 2).
